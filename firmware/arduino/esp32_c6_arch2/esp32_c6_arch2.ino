...
// Double-buffering structure to avoid race conditions when updating from
// network
...
#endif
  if (is_master_clock)
    return; // Master ignores incoming clocks
  ...
  if (len == sizeof(sync_message_t)) {
    ...
    if (msg.magic == SYNC_MAGIC) {
      ...
      if (!clockSynced) {
        ...
        clockSynced = true;
        Serial.println(
            "[ESP-NOW] Initial hardware clock synchronized to Grandmaster!");
      } else {
        Serial.print(">");
        clockOffsetUs = instantOffsetUs;
      }
...
void configureEspNow() {
  ...
  if (is_master_clock) {
    Serial.println(
        "[ESP-NOW] Configuring as GRANDMASTER. Adding broadcast peer.");
    ...
    Serial.printf(
        "[ESP-NOW] Configuring as FOLLOWER on channel %d. Listening...\n",
        master_channel);
  }
}
    ...
    if (payload != lastPayload) {
      ...
      if (!err) {
        ...
        if (new_is_master != is_master_clock ||
            new_master_channel != master_channel || !esp_now_initialized) {
        ...
        // Atomic swap of buffers makes it immediately live to the execution
        // engine
        activeConfigIdx = nextIdx;
        sequenceRunning = nxCfg->isValid;
        lastPayload = payload;

        Serial.printf("[config] SUCCESS! New config applied: %d states, %u ms "
                      "total loop\n",
...
void stateMachineEngineTask(void *pvParameters) {
  while (true) {
    ...
    if (sequenceRunning && cfg->isValid && clockSynced) {
      ...
      // Calculate delay. Don't block indefinitely to stay responsive to
      // updates.
      uint32_t waitUs = timeToNextStateUs;
      if (waitUs > 10000)
        waitUs = 10000; // sleep 10ms at most to pick up new configs instantly
...
void setup() {
  ...
  xTaskCreate(syncBroadcastTask, "syncTask", 3072, NULL, 3,
              &syncBroadcastTaskHandle);

  // Create Engine Task
  xTaskCreate(stateMachineEngineTask, "sm_engine", 4096, NULL,
              configMAX_PRIORITIES - 1, &engineTaskHandle);

  Serial.println(
      "[boot] Engine & Sync Tasks running. Waiting for clock sync...");
}
